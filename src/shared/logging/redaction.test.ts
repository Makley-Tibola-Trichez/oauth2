...
describe('redaction', () => {
  ...
  for (const campo of camposSensiveis) {
    it(`redige o campo sensível "${campo}"`, () => {
  ...
  const classificacoes: [string, boolean][] = [
    ['clientSecret', true],
    ['CLIENT_SECRET', true],
  ...
  for (const [campo, sensivel] of classificacoes) {
    it(`classifica "${campo}" como sensível=${sensivel}`, () => {
